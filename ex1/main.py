...
def segment_multi_channel(x: np.ndarray, p: float, q: int):
    ...
    Returns:
        ...
        c: Total cost value of optimal segmentation
    """
    n, d = x.shape  # Number of points and number of channels (dimensions)
    c = np.full(n + 1, np.inf, dtype=np.float32)
    c[0] = 0  # Base case: zero cost to segment an empty prefix
    t = np.zeros(n + 1, dtype=int)

    # Precompute cumulative sums and cumulative sums of squares for efficiency
    cumulative_sum = np.cumsum(x, axis=0, dtype=np.float32)
    cumulative_sum_squared = np.cumsum(x ** 2, axis=0, dtype=np.float32)

    def calc_sum_s_multi(j, i):
        # Calculate sum and sum of squares for multi-dimensional segments
        sum_s = cumulative_sum[j - 1] - (cumulative_sum[i - 1] if i > 0 else 0)
        sum_s2 = cumulative_sum_squared[j - 1] - (cumulative_sum_squared[i - 1] if i > 0 else 0)
        return sum_s, sum_s2

    # Calculate cost for a multi-dimensional segment
    def segment_cost_multi(i, j):
        segment_len = j - i
        sum_s, sum_s2 = calc_sum_s_multi(j, i)
        mean_s = sum_s / segment_len
        sse = np.sum(sum_s2 - 2 * mean_s * sum_s + segment_len * mean_s ** 2)
        return sse

    # Dynamic programming to compute minimum cost
    for j in range(1, n + 1):
        for i in range(max(0, j - q), j):
            seg_cost = segment_cost_multi(i, j) + (p if i != 0 else 0)
            if c[i] + seg_cost < c[j]:
                c[j] = c[i] + seg_cost
                t[j] = i

    # Backtrack to retrieve segments
    s = []
    j = n
    while j > 0:
        i = t[j]
        segment_len = j - i
        sum_s, _ = calc_sum_s_multi(j, i)
        mean_s = sum_s / segment_len
        s.append([i + 1, j, mean_s.mean()])  # Store the mean across all dimensions
        j = i
